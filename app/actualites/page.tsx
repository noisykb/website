        ...
        <section className="py-14 md:py-24">
          <div className="site-shell grid gap-px overflow-hidden border border-[#d9d5e0] bg-[#d9d5e0] md:grid-cols-2">
            {articles.map((article, index) => (
              <article key={article.slug} className="group relative min-h-72 bg-[#f7f6fb] p-7 transition-colors hover:bg-white md:p-10">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="grid size-10 place-items-center rounded-full border-2 border-[#09090d] transition-colors group-hover:bg-[#cba6ff]"><ArrowUpRight size={18} aria-hidden="true" /></span>
                </div>
                <p className="mt-12 text-xs font-black uppercase tracking-[0.12em] text-[#6a6d7b]">Match {String(articles.length - index).padStart(2, '0')}</p>
                ...
                {article.excerpt ? <p className="mt-4 line-clamp-2 text-sm text-[#5e6474]">{article.excerpt}</p> : null}
              </article>
            ))}
          </div>
        </section>
