          ...
          <div className="site-shell relative z-10 grid min-h-[680px] items-end gap-12 pb-16 pt-24 lg:grid-cols-[1fr_320px] lg:pb-20">
            ...
            <aside className="border-l-4 border-[#8b2ad1] bg-[#07070a]/45 p-6 backdrop-blur-sm lg:mb-2" aria-label="Informations pratiques">
              ...
              <dl className="mt-8 grid gap-5 text-sm">
                <div className="flex gap-3"><Timer className="mt-0.5 text-[#cba6ff]" size={19} aria-hidden="true" /><div><dt className="font-bold">Entraînement</dt><dd className="text-white/70">Lundi · 20h30–22h30</dd></div></div>
                <div className="flex gap-3"><MapPin className="mt-0.5 text-[#cba6ff]" size={19} aria-hidden="true" /><div><dt className="font-bold">Gymnase Marcel Cerdan</dt><dd className="text-white/70">Noisy-le-Grand</dd></div></div>
              </dl>
            </aside>
          </div>
        </section>
        ...
        <section className="bg-white py-16 md:py-24">
          <div className="site-shell grid gap-12 lg:grid-cols-[0.8fr_1.4fr]">
            <div>
              ...
              <p className="mt-6 max-w-md text-[#4d5161]">Trois équipes de quatre joueurs partagent le même terrain. Pour gagner, il faut observer, coopérer et garder l’énorme ballon en mouvement.</p>
            ...
            <ol className="grid gap-px bg-[#bbb3c8] sm:grid-cols-3">
              {[
                ...
                <li key={number} className="min-h-64 list-none bg-[#f7f6fb] p-7 sm:min-h-80">
                  ...
                  <p className="mt-4 text-sm leading-relaxed text-[#555969]">{description}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section className="py-20 md:py-28">
          <div className="site-shell">
            <div className="flex flex-col justify-between gap-6 border-b-2 border-[#09090d] pb-7 md:flex-row md:items-end">
              <div><p className="eyebrow text-[#7412b9]">Sur le terrain</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em] md:text-6xl">Les dernières nouvelles</h2></div>
            ...
            <div className="grid md:grid-cols-3">
              {news.map((item) => (
                <article key={item.href} className="group relative border-b border-[#c3c9cc] py-8 md:border-b-0 md:border-r md:px-7 md:first:pl-0 md:last:border-r-0">
                  <p className="font-mono text-xs font-bold text-[#7412b9]">{item.date}</p>
                  ...
                  <span className="mt-8 inline-flex size-10 items-center justify-center rounded-full border-2 border-[#09090d] transition-colors group-hover:bg-[#cba6ff]"><ArrowRight size={18} aria-hidden="true" /></span>
                </article>
              ))}
            </div>
          </div>
